...
type
  ...
  public
    ...
    function OrganizeOrder(userId:Integer;userNaim:string;Msg:AnsiString):Double ;
  end;
...
var task,userId,uId,role,otdelId,schetId,iTmp,stolNum:Integer;
  passw,answ,userNaim,uNaim,sTmp,s,ss,sstolNum,sAllOrderId,skolvo:AnsiString;
  orderSumm,kolvo:Double;
  dt:TDateTime;
  opened:string;
  uFound:Boolean;
begin
  ...
  begin
    ...
    if (role=1) or (role=3) then begin m('Запрос не от менеджера',clRed);Exit;end;
    if uFound then
    begin
...
//*********************далее работаем с task>0*********************************
  ...
  case task of
    1:begin //запрос: 1;userId;
        ...
        s:='';
        if userId=0 then
          ss:='SELECT ALLORDERID,ORDERID,STOL,OPENED,CLOSED FROM TORDERS'+
...
//        if stolNum>150 then stolNum:=1;
        ...
        begin
          Active:=False;
          SQL.Text:='SELECT m.BLUDID,m.KOLVO,m.PRICE,b.NAIM,b.TYPE FROM TMOVES m'+
    ...
    3:begin
        ...
        schetId:=StrToInt(FloatToStr(OrganizeOrder(userId,userNaim,Msg)));
...
//          SQL.Text:='SELECT BLUDID,NAIM,PRICE where issale>0 and upper (NAIM) like upper ('+qs('%'+s+'%')+')';
          Active:=True;
          if not IsEmpty then
          begin
            answ:=answ+FieldByName('BLUDID').AsString+';'+FieldByName('NAIM').AsString+';'+
              FieldByName('PRICE').AsString+';'+FieldByName('TYPE').AsString+';';
          end;
        end;
        //answ:=answ+'Ok;';
      end;
    5:begin
        ...
        begin
          Active:=False;
          SQL.Text:='SELECT BLUDID,NAIM,PRICE,TYPE FROM TBLUDS WHERE PRR='+I2S(iTmp)+
            ' AND AVAIL>0 AND ISSALE=1 ORDER BY NAVBAT,NAIM';  ///NAIM
          Active:=True;
          First;while not Eof do
          begin
            sTmp:=sTmp+FieldByName('BLUDID').AsString+','+FieldByName('NAIM').AsString+','+
            FieldByName('PRICE').AsString+','+FieldByName('TYPE').AsString+'|';
            Next;
          end;
        end;
    ...
    6:begin
        ...
        if((iTmp>100) and (FirstI('SELECT ISREZERV FROM TKABINS WHERE IDKABIN='+I2S(iTmp))>0)) then
        begin
    ...
    7:begin //запрос: 1;userId;
        ...
        s:='';
        ss:='SELECT ALLORDERID,ORDERID,STOL,OPENED,CLOSED FROM TORDERS'+
            ' WHERE USERID>0 AND CLOSED>'+QS(DT2S(0))+' ORDER BY OPENED DESC';
        with dm.qtemp do
        begin
          Active:=False; SQL.Text:=ss; Active:=True; First;
          while not Eof do
...
function TserverForm.OrganizeOrder(userId:Integer;userNaim:string;Msg: AnsiString): Double;
var
  zakazs:array[1..100] of TZakaz;
  zCnt,schetId,stolNum,iTmp:Integer;
  tipprr,i,k,prr,kSumm,ORDERID:Integer;
  otdels:array[1..OTDELSCNT] of Integer;
  pr:TStringList;
  s,opened,closed,id,idAll,fmt,sTmp:string;
  closedDT:TDateTime;
  allsumm1,allsumm2,allSumm,wSumm,iTmp2:Double;
begin
  //3;2;0;1;1;1;3;2;4;4;
  ...
  Result:=0;
  schetId:=explodeInt(Msg);if schetId<0 then begin m('Неверный счет',clRed); Exit;end;
  if(schetId>0) then
  begin
...
//        if stolNum>150 then stolNum:=1;
  ...
  begin
    ...
    sTmp:=explodeStr(Msg);  if(sTmp<='0') then Break;  zakazs[zCnt+1].bludId:=StrToInt(sTmp);
    sTmp:=explodeStr(Msg);  if (sTmp='0') then Break;  zakazs[zCnt+1].kolvo:=StrToFloat(sTmp);
    with dm.qwork do
    begin
      Active:=False;
...
//    if (ORDERID>=300) or (formMain.genID>=formMain.SurogID) then
//    begin MessageDlg(
...
//    Exit; end;
    ...
    s:='INSERT INTO TORDERS (ALLORDERID,ORDERID,USERID,STOL,OPENED,CLOSED,PLASTIK,WSUMM,KABINASUMM,RESERV,BAND)'+
  ...
  begin
    ...
    s:='INSERT INTO TMOVES (ALLORDERID,BLUDID,KOLVO,PRICE,SUSERID) VALUES ('+idAll+','+
      I2S(zakazs[i].bludId)+','+FormatFloat('0.###',zakazs[i].kolvo,tf)+','+I2S(zakazs[i].price)+
      ','+I2S(userId)+')';
    ExecQ(s);
  end;
  ...
  begin
    ...
    for i := 1 to zCnt do
      ...
      then begin
        ...
        s:=Format(PRNFORMAT,[zakazs[i].naim,FloatToStr(zakazs[i].kolvo)]);pr.Add(s);
      end;
    s:=PRNRAZDEL; pr.Add(s);
  ...
  begin
    ...
    ExecQ('UPDATE TORDERS SET WSUMM='+FormatFloat('0.###',wSumm,tf)+',KABINASUMM='+I2S(kSumm)+' WHERE ALLORDERID='+idAll);
    Result:=StrToInt(id);
